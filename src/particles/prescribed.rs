...
fn orbit_source<V : Into<Vec2D>>(scale: &Scale, rect: Rect, velocity: V) -> RandomParticleSource {
    ...
    RandomParticleSource::new(scale.rect_source(rect), modulation)
        ...
        .with_velocity((velocity.clone(), velocity * 0.5))
        .with_alpha((0.9, 0.1))
        .with_orbit((0.5, 0.5))
}
...
fn perimeter_sources(scale: &Scale, rect: Rect, color: ParticleColor) -> [RandomParticleSource; 4] {
    ...
    [
        RandomParticleSource::burst(top, SPRITE, color, (Vec2D::new(0.0, -V), Vec2D::new(0.2, 0.1)), FADE_OUT, ALPHA),
        RandomParticleSource::burst(right, SPRITE, color, (Vec2D::new(V, 0.0), Vec2D::new(0.1, 0.2)), FADE_OUT, ALPHA),
        RandomParticleSource::burst(bottom, SPRITE, color, (Vec2D::new(0.0, V), Vec2D::new(0.2, 0.1)), FADE_OUT, ALPHA),
        RandomParticleSource::burst(left, SPRITE, color, (Vec2D::new(-V, 0.0), Vec2D::new(0.1, 0.2)), FADE_OUT, ALPHA),
    ]
}
