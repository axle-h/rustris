...
impl<T : Clone> VariableQuantity<T> {
    pub fn new(quantity: T, variance: T) -> Self {
        Self { rng: thread_rng(), quantity, variance }
    }
...
impl VariableQuantity<Vec2D> {
    pub fn next(&mut self) -> Vec2D {
        self.quantity + Vec2D::new(self.variance.x() * self.rand_signed_f64(), self.variance.y() * self.rand_signed_f64())
    }
}
...
impl VariableQuantity<ParticleWave> {
    pub fn next(&mut self) -> ParticleWave {
        let magnitude = self.quantity.magnitude() + self.variance.magnitude() * self.rng.gen::<f64>();
        let frequency = self.quantity.frequency() + self.variance.frequency() * self.rng.gen::<f64>();
...
impl<T : Clone> ProbabilityTable<T> {
    pub fn new() -> Self {
        Self { rows: vec![], rng: thread_rng(), total: 0.0 }
    }

    pub fn identity(value: T) -> Self {
        Self { rows: vec![], rng: thread_rng(), total: 0.0 }.with(value, 1.0)
    }
