...
impl MetricSnips {
    pub fn right<P : Into<Point>>(point: P, max_value: u32) -> Self {
        Self { point: point.into(), max_value, max_chars: char_length(max_value), align: FontAlign::Right }
    }

    pub fn zero_fill<P : Into<Point>>(point: P, max_value: u32) -> Self {
        Self { point: point.into(), max_value, max_chars: char_length(max_value), align: FontAlign::Left { zero_fill: true } }
    }

    pub fn left<P : Into<Point>>(point: P, max_value: u32) -> Self {
        Self { point: point.into(), max_value, max_chars: char_length(max_value), align: FontAlign::Left { zero_fill: false } }
    }

    pub fn offset(&self, x: i32, y: i32) -> Self {
        Self { point: self.point.offset(x, y), max_value: self.max_value, max_chars: self.max_chars, align: self.align }
    }

    fn to_zero_fill(&self) -> Option<u32> {
        match self.align {
...
pub enum FontRenderOptions {
    Sprites { file: String, sprites: Vec<FontSprite>, spacing: u32 },
}

impl FontRenderOptions {
    pub fn build<'a>(&self, texture_creator: &'a TextureCreator<WindowContext>) -> Result<FontRender<'a>, String> {
        match self {
            FontRenderOptions::Sprites { file, sprites, spacing } =>
                FontRender::from_sprites(texture_creator, &file, sprites.clone(), *spacing)
        }
    }
}
...
impl<'a> FontRender<'a> {
    pub fn from_sprites(
        ...
        let mut texture = texture_creator.load_texture(sprite_file)?;
        texture.set_blend_mode(BlendMode::Blend);
        let sprites = sprites.iter()
            .map(|&x| (x.value, x.snip))
            .collect();
        Ok(Self { texture, sprites, spacing })
    }
    ...
    pub fn from_font(
        ...
        let chars = ('A'..='Z')
            ...
            .chain('0'..='9')
            .chain([' ', ',', '.'])
            .map(|c| (c, FontTexture::from_char(&font, texture_creator, c, color).unwrap()))
        ...
        Ok(Self { texture, sprites, spacing: 0 })
    }

    pub fn render_string(&self, canvas: &mut WindowCanvas, dest: Point, value: &str) -> Result<(), String> {
        let mut dest = dest;
    ...
    pub fn render_number(&self, canvas: &mut WindowCanvas, meta: MetricSnips, value: u32) -> Result<(), String> {
