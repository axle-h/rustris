...
pub enum TetrominoScaleType {
    ...
    Fill { default_scale: f64, peek0_scale: f64 },
...
impl<'a> Theme<'a> {
    ...
    pub fn background_size(&self) -> (u32, u32) { self.background_size }
    ...
    pub fn draw_background(&mut self, canvas: &mut WindowCanvas, game: &Game) -> Result<(), String> {
        ...
        for (index, (peek_shape, peek_rect)) in metrics.queue.iter().copied().zip(self.peek_snips).enumerate() {
            self.draw_tetromino(canvas, peek_shape, peek_rect, index == 0)?;
        }

        self.font.render_number(canvas, self.score_snip, metrics.score)?;
        self.font.render_number(canvas, self.level_snip, metrics.level)?;
        self.font.render_number(canvas, self.lines_snip, metrics.lines)?;
    ...
    pub fn draw_board(
        ...
        canvas.set_draw_color(Color::RGBA(0, 0, 0, 0));
        canvas.clear();
        canvas.copy(&self.board_texture, None,Rect::new(0, 0, self.board_snip.width(), self.board_snip.height()))?;
        ...
        if render_board {
            self.sprite_sheet.draw_board(canvas, game, &self.geometry, self.ghost_mino_type)?;
